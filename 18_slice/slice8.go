...
func main() {
	...
	slice2[1] = 200
	fmt.Println(slice1) // slice1도 변하는 영향을 받음
	fmt.Println(slice2)

	slice1 = append(slice1, -1, -2, -3, -4)                           // 기존 slice1의 공간이 충분하지 않아 새로 할당
	fmt.Println(slice1, slice1Ptr.Data, slice1Ptr.Len, slice1Ptr.Cap) // slice1은 새로운 배열을 가리킴
	fmt.Println(slice2, slice2Ptr.Data, slice2Ptr.Len, slice2Ptr.Cap) // slice2는 기존의 배열을 가리킴
